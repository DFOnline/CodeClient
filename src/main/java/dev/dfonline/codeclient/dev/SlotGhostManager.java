...
public class SlotGhostManager {
    ...
    public static void drawSlot(DrawContext context, Slot slot) {
        ...
        if(!(CodeClient.MC.currentScreen instanceof GenericContainerScreen || CodeClient.MC.currentScreen instanceof CustomChestMenu)) action = null;
        if(action == null || action.icon == null || action.icon.arguments == null || slot.inventory instanceof PlayerInventory) return;
