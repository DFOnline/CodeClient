    ...
    public LiteralArgumentBuilder<FabricClientCommandSource> create(LiteralArgumentBuilder<FabricClientCommandSource> cmd, CommandBuildContext registryAccess) {
        return cmd.then(argument("path", greedyString()).suggests(this::suggestTemplates).executes(context -> {
            ...
            if (!path.toAbsolutePath().startsWith(FileManager.templatesPath().toAbsolutePath())) {
                Utility.sendMessage(Component.translatable("codeclient.files.not_in_templates"), ChatType.INFO);
                return -2;
            }
            if (Files.notExists(path)) {
                Utility.sendMessage(Component.translatable("codeclient.files.error.not_found"), ChatType.FAIL);
                return -1;
            }
            try {
                FileUtils.forceDelete(path.toFile());
                Utility.sendMessage(Component.translatable("codeclient.files.deleted", path), ChatType.SUCCESS);
            } catch (Exception e) {
                Utility.sendMessage(Component.translatable("codeclient.files.error.cant_delete"), ChatType.FAIL);
            }
            return 0;
        }));
    }
